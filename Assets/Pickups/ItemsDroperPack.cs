		...
		for(int i=0;i<Number;i++){
			if(Item!=null){
				...
				if(item.GetComponent<Rigidbody>()){
					item.GetComponent<Rigidbody>().AddForce((-this.transform.forward + Vector3.up) * Force);
					item.GetComponent<Rigidbody>().AddTorque((-this.transform.forward + Vector3.up) * Force);	
				}
